...
const OrderBaikal: FC<Props> = (props) => {
    const {
        baikalTrackerLink,
        baikalTrackerHistory,
        orderId,
        isContainer = false
    } = props
    ...
    const onClickHandler = () => {
        const orderId = window.location.pathname.replace('/container/', '')
    ...
    let content = (
        <>
            <p className={classes.text}>
                Введите идентификатор груза от сервиса BAIKAL
            </p>
            <SvgBaikalStart />
            <input
                value={baikalId}
    ...
    if (baikalTrackerLink && baikalTrackerHistory.length !== 0) {
        ...
        content = (
            <>
                <p className={classes.track}>
                    Код:{' '}
                    {baikalTrackerLink.replace(
                        'https://tms.baikalasia.com/selectBaikalasiaE.aspx?CNum=',
                        ''
                    )}
                </p>
                <p className={classes.track}>Идентификатор {id}</p>
                ...
                <ul className={classes.list}>
                    {[...baikalTrackerHistory].reverse().map(({date, text}) => {
                        return (
                            <li key={date + text}>
    ...
    return (
        <div className={classes.card}>
            <p className={classes.title}>TRACKING</p>
            {content}
        </div>
    )
}
