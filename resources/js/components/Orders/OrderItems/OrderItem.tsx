...
const OrderItem: React.FC<Props> = props => {
    ...
    useEffect(() => document.querySelector(`.${classes.productCount}`)?.focus(), [])

    const onKeyDownHandler = (e) => {
        if (e.key === 'Enter') {
        e.preventDefault()
        const arr = Array.from(document.querySelectorAll(`.${classes.productCount}`))
        const inputIndex = arr.indexOf(e.target)
        const nextInputIndex = inputIndex + 1 < arr.length ? inputIndex + 1 : inputIndex
        document.querySelectorAll(`.${classes.productCount}`)[nextInputIndex].focus()
        }
    }

    return (
        <div key={item.id + item.nameRu} className={classes.products + ' row mb-2'}>
            ...
            <div className='col-xl-4 pr-xl-0'>
                <div className={classes.productPrices}>
                    <Input
                        ...
                        onKeyDown={(e) => onKeyDownHandler(e)}
                    />
                    {item.price ? moneyFormatter(item.price, ['cny']) : null}
                </div>
            </div>
            <div className={classes.deleteButton + ' col-xl-1 text-right pr-xl-4 pr-2'}>
                <SvgClose onClick={() => onDelete(item.id)} className='mt-lg-3 mt-0 deleteButton' />
            </div>
        </div>
    );
};
