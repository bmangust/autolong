              ...
              CREATE DEFINER=`$userName`@`localhost` TRIGGER `$database`.`products_BEFORE_UPDATE` BEFORE UPDATE ON `products` FOR EACH ROW
                  BEGIN
                    IF OLD.published = 0 AND NEW.published = 1 THEN
                        SET NEW.image = CONCAT('$fullPathImage', NEW.image);
                        ...
                        VALUES (NEW.name_ru, NEW.name_en, NEW.about_ru, NEW.about_en, NEW.price_rub, NEW.price_usd, NEW.price_cny, NEW.weight_netto, NEW.weight_brutto, NEW.image, NEW.autolong_number, NEW.vendor_code);
                    END IF;
                  END
        ");
        }
